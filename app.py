...
def build_features_and_risk(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, r in df.iterrows():
        ...
        if pd.notna(r.get("price_change_percentage_1h")) and r["price_change_percentage_1h"] < -0.5: why.append("1h downside")
        if pd.notna(r.get("volume_ratio")) and r["volume_ratio"] > 0.5: why.append("Heavy volume vs mcap")
...
if df.empty:
    ...
    st.write("Try **Data source → primary** (or **offline** if you committed a snapshot) and use the sidebar **Refresh offline snapshot** button.")
...
with left:
    ...
    selected_symbol = st.selectbox("Pick a coin", symbols, index=0 if symbols else None)
with right:
    analyze_clicked = st.button("Analyze", use_container_width=True)

if analyze_clicked and symbols:
